...
function EntityListRow({ ent, selected, onSelect }) {
  ...
  return (
    <div className={'entity-row' + (selected ? ' selected' : '') + (ent.destroyed ? ' dead' : '')}
      ...
      <div className="er-label">{shortId(ent.id)}</div>
          ...
          {sim.engagements.map((e, i) => {
            ...
            return (
              <div key={i} className="eng-row">
                ...
                <span style={{ color: FAC_COLORS[b.faction] }}>{shortId(b.id)}</span>
              </div>
            );
          })}
        </div>
  ...
  return (
    <div className="insp">
      <div className="insp-id-row">
        <span className="insp-id">{shortId(ent.id)}</span>
  ...
  return (
    <div className="hover-card" style={{ left: pos.x + 16, top: pos.y - 50 }}>
      <div className="hc-head">
        <span className="hc-id">{shortId(ent.id)}</span>
    ...
    <div className="event-ticker">
      {recent.map((e) => {
        const cls = e.type === 'entity.destroyed' ? 'red'
                  ...
                  : '';
        let label;
        if (e.type === 'engagement.fire') label = `${shortId(e.data.shooter || e.data.entity_a)} fires`;
        else if (e.type === 'unit.destroyed') label = `${shortId(e.data.entity_id)} destroyed`;
        else if (e.type === 'entity.destroyed') label = `${shortId(e.data.entity_id || e.data.id)} destroyed`;
        else if (e.type === 'entity.created') label = `${shortId(e.data.entity_id || e.data.id)} deployed`;
        else if (e.type === 'entity.damaged') label = `${shortId(e.data.entity_id || e.data.id)} took ${(e.data.damage || e.data.dmg || 0).toFixed(1)} dmg`;
